    ...
    for coupling_type in ["linear", "nonlinear", "periodic"]:
        for activation_type in ["none", "mixed"]:
            ...
            # Benchmark is only for each category, and noise is noise 2.00 
                    print(f"Running for coupling={coupling_type}_noise=0.00_systems={systems_num}_confounder=False_standardize=False_timelag=1_activation={activation_type}")

                    # Load lorenz84 for hyperparameter search
                    data_dir_noise = Path(f"/home/mila/j/julien.boussard/scratch/CausalDynamics/data/coupled/coupling={coupling_type}_noise=0.00_systems={systems_num}_confounder=False_standardize=False_timelag=1_activation={activation_type}/data")
                    eval_dir_noise = Path(f"/home/mila/j/julien.boussard/scratch/CausalDynamics/data/coupled/coupling={coupling_type}_noise=0.00_systems={systems_num}_confounder=False_standardize=False_timelag=1_activation={activation_type}/eval/{model_name}")
                    ...
                    for file in all_files[:5]: # Just run on first 5 datasets
                        ...
                        for j, x in enumerate(timeseries[:, :, None]):
                            ...
                            trained_model, mask = lowrank_model.run(n_samples=3, lambda_grad=100, X=torch.tensor(x).to(device), l0_l1_l2='l0', th=0.5, lr=0.001, lambda_m = 0.025, n_inner_min_sparse = 100, batch_size=128, n_inner=4_000, patience = 200, save_fig_path="coupled_timelag_loss.png")
                            mask_numpy = mask.detach().cpu().numpy()

                            #For time lag evaluation
                            mask_numpy = mask_numpy.max(0)[None]
