...
if __name__ == "__main__":
    ...
    print(f"Running for noise ENSO")

    # Load lorenz84 for hyperparameter search
    data_dir_noise = Path(f"/home/mila/j/julien.boussard/scratch/CausalDynamics/data/climate/coupled_enso_modes/data")
    eval_dir_noise = Path(f"/home/mila/j/julien.boussard/scratch/CausalDynamics/data/climate/coupled_enso_modes/eval/{model_name}")
    ...
    final_lambda_m = 0.025

    all_files = glob.glob(str(data_dir_noise / "*.nc"))
    print("ALL FILES")
    print(all_files)
    for file in all_files: # Just run on first 10 datasets
