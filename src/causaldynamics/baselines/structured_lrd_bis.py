...
class StructuredLowRankDiscovery():
    ...
    def run(self,
            ...
            th: float = 0.5, 
            plot_frequency: int = 10,) -> StructuredDynamics:
        """
        ...
        print(f"X.mean(dim=0) = {X.mean(dim=0)}")
        print(f"X.std(dim=0) = {X.std(dim=0)}")
        ...
        while outer < n_outer:
            ...
            for iter in range(n_inner):
                ...
                #     idx = torch.randperm(n_samples, device=X_t_all.device)[:batch_size]
                #     X_t = X_t_all[idx].squeeze(1)
                #     X_last = X_last_all[idx].squeeze(1)
                # else:
                #     X_t = X_t_all
                #     X_last = X_last_all
                ...
                # X_t = X_t.detach().requires_grad_(True)
                    ...
                    # print(input_grads) # require grad = True?

                    # Gradient penalty — gradient norm should be <= L
    ...
    #                 nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)
                ...
                history.append({
                    ...
                    "w_sparse": w_sparse_all / n_batches / self.model.k**2,
